...
const ResultPage = () => {
    ...
    return (<div>
        {data.assignments_assignment_by_pk.invites.map(invite => <>
            <div className="lg:flex lg:items-center lg:justify-between">
                <div className="flex-1 min-w-0">
                    <h2 className="text-xl font-semibold leading-7 text-gray-800 sm:text-2xl sm:leading-9 sm:truncate">
                        {invite.is_public ? moment(invite.created_at).format("MMM DD, YYYY") : "AP Calculus Period 2"} <span className="font-light">({invite.join_code})</span>
                    </h2>
    ...
    <span className="hidden sm:block shadow-sm rounded-md">
      ...
      <button type="button"
        ...
        <svg className="-ml-1 mr-2 h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          <path
                ...
                <div className="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                    <div
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead>
                            <tr>
                                {invite.is_public ? null : <th className="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                                    Name
                                </th>}
                            ...
                            <tbody>
                            {invite.submissions.map((submission, index) => {
                                if (index % 2 == 0) {
                                    return (submission.scoreReportByScoreReport ? <tr className="bg-gray-50">
                                        {invite.is_public ? null : <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
                                            Cody Fisher
                                        </td>}
